...
open class TabPageViewController: UIPageViewController {
    ...
    var childView:((_ page:Int)->Void)?
    
    var currentIndex: Int? {
        guard let viewController = viewControllers?.first else {
            return nil
        }
        let page =  tabItems.map{ $0.viewController }.index(of: viewController)
...
//            navigationBar.shadowImage = UIImage()
            ...
            let statusBarView = UIView.init(frame: CGRect(x: 0, y: -20, width: screenW, height: 20));
